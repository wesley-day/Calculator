...
def parse_primary(toks):
    tok = toks[0]
    if tok[0] == "LPAREN":
        toks_left, expr = parse_additive(toks[1:])
        toks_left = match_tok("RPAREN", toks_left)
        return toks_left, expr 
    if tok[0] == "FUN":
        ...
        for i in range(Function.num_params[tok[1]]):
            ...
            toks_left, expr = parse_additive(toks_left)
            exprs.append(expr)
        toks_left = match_tok("RPAREN", toks_left)
        return toks_left, Function(tok[1], exprs)
    if tok[0] == "CONST":
        return toks[1:], Value(constants[tok[1]])
    if tok[0] == "NUM":
        return toks[1:], Value(convert_to_number(tok[1]))
    if tok[0] == "SUB":
        toks_left, expr = parse_primary(toks[1:])
        expr.neg = -1
        return toks_left, expr
    raise ParseError("Invalid syntax")

def parse_factorial(toks):
    toks, expr = parse_primary(toks)
    if not toks or toks[0][0] != "FACT":
        return toks, expr
    return toks[1:], Function("fact", [expr])

def parse_exponential(toks):
    toks1, expr1 = parse_factorial(toks)
    if not toks1 or toks1[0][0] != "POW":
        return toks1, expr1
    toks2, expr2 = parse_exponential(toks1[1:])
    return toks2, Binop("POW", expr1, expr2)

def parse_multiplicitive(toks):
    toks1, expr1 = parse_exponential(toks)
    ...
    toks2, expr2 = parse_multiplicitive(toks1[1:])
    return toks2, Binop(op, expr1, expr2)

def parse_additive(toks):
    toks1, expr1 = parse_multiplicitive(toks)
    if not toks1 or toks1[0][0] != "ADD" and toks1[0][0] != "SUB":
        return toks1, expr1
    op = "ADD" if toks1[0][0] == "ADD" else "SUB"
    toks2, expr2 = parse_additive(toks1[1:])
    return toks2, Binop(op, expr1, expr2)
...
def parse(toks):
    ...
    toks_left, expr = parse_additive(toks)
    if toks_left:
        raise ParseError("Invalid syntax")
    return expr